...
from app.infra.repository.cat import (
    CatUpdate,
    find_cats_by_task_id,
    get_image,
    update_cats,
)
...
def classify_cat_task(
    ...
    try:
        ...
        if not cats:
            update_tasks_status(
                ...
                [
                    TaskStatusUpdate(
                        taskId=taskId,
                        status=TaskStatus.CLASSIFY_CAT_FINISHED,
                        hasCat=None,
                    ),
                ],
            )
        ...
        with ThreadPoolExecutor() as executor:
            images = list(
                executor.map(lambda cat: get_image(r2Client, cat.catImageUrl), cats),
            )
        ...
        update_tasks_status(
            ...
            [
                TaskStatusUpdate(
                    taskId=taskId,
                    status=TaskStatus.CLASSIFY_CAT_FAILED,
                    hasCat=None,
                ),
            ],
        )
